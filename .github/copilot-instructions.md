...
### ~~Correct Automated Deployment with GitHub Actions~~ (DISABLED - January 3, 2026)
...
### ALTERNATIVE: ~~Correct~~ Manual Deployment with robocopy (SUPERSEDED - January 3, 2026)
...
## 🌐 WEB DEPLOYMENT TO GITHUB PAGES (SIMPLIFIED - January 3, 2026)

**IMPORTANT:** GitHub Pages serves files from the **ROOT** of the gh-pages branch, NOT from `build/web/` folder.

**One Branch Workflow (gh-pages only)**

To simplify the deployment process, we are now working exclusively on the `gh-pages` branch.

**Deployment Steps:**

1.  **Make Changes:** Modify the code as needed in VS Code.
2.  **Build**: From the project root, build the web app:
    ```powershell
    flutter build web --release --base-href=/
    ```
3.  **Commit and Push**: Commit all the changes and push them to the `gh-pages` branch.
...
*Note: This workflow eliminates branch switching and reliance on GitHub Actions for deployment.*
